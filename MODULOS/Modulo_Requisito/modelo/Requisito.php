...
class Requisito
{
	...
	public function registrosrequisito( $array = array() )
	{
		...
		$sqlite ="SELECT id_requisito , nombre_requisito,estatus FROM pasantias.requisito WHERE nombre_requisito LIKE upper('$requisito%') ORDER BY nombre_requisito;"; 

		$sqliteestatus ="SELECT id_requisito , nombre_requisito,estatus FROM pasantias.requisito WHERE estatus = '$estatus' ORDER BY nombre_requisito;"; 
		$execute="";
		if($requisito == '')
		{
			$execute = $sqliteestatus;
		}else 
		{
			$execute = $sqlite;
		}
